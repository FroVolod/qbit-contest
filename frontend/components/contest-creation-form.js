import { Col, Button, Form, FormGroup, Label, Input, Text, InputGroup, InputGroupAddon, InputGroupText, Spinner } from 'reactstrap';
...
export default class extends React.Component {
    ...
    componentDidMount() {
        ...
        this.getParticipantsGroups(this.state.author)
            ...
            .catch((e) => (
                console.log('componentDidMount: setting the contestParticipans error -- ', e)
            ))
    ...
    componentDidUpdate(nextProps) {
        ...
        this.getParticipantsGroups(this.state.author)
            .then((participantsGroups) => {
                console.log('DidUpdate getParticipantsGroups response: ', participantsGroups)
                if (participantsGroups === this.state.participantsGroups) return
                console.log('componentDidUpdate: setting state.conestParticipantsGroups')
                this.setState(
                    {
                        participantsGroups,
                        showParticipantsGroupsSpinner: false
            ...
            .catch((e) => (
                console.log('componentDidUpdate: setting the contestParticipans error -- ', e)
            ))
    ...
    createContest = async () => {
        ...
        let allowLanguages = this.state.allowLanguages.filter(lang => this.state.programmingLanguagesEnabled[lang])
    ...
    onAuthorChange = (e) => {
        ...
        this.getParticipantsGroups(e.target.value)
            ...
            .catch((e) => (
                console.log('onAuthorChange: setting the contestParticipans error -- ', e)
            ))
    }
    ...
    getParticipantsGroups = async (author) => {
        ...
        const participantsGroups = await this.props.state.session.call('com.demo.get-participants-groups', [author,])
    ...
    setContestParticipantsGroups = (e) => {
        console.log('Setting the setContestParticipantsGroups with: ', e.target.checked, e.target.id)
    ...
    getTitlesProblemsAndInfo = (contest) => {
        ...
        const item = this.state.contestsDict.find(item => item.contest_title == contest)
    ...
    onChangeAllowLanguagesList = (e) => {
        console.log('onChangeAllowLanguagesList: ', e.target.id, e.target.checked, this.state.programmingLanguagesEnabled)
        ...
        return (
            <Form>
                ...
                <FormGroup row>
                    ...
                    <Col sm={6}>
                        <Input type="textarea" name="text" id="author_id" placeholder={this.state.author} onBlur={this.onAuthorChange}>
                            {this.state.author}
                        </Input>
                    </Col>            
                </FormGroup>
                        ...
                        <Col sm={6}>
                            {this.state.participantsGroups.map((group) => (
                                <InputGroup key={group}>
                                    ...
                                    <InputGroupText>
                                        <Input id={group} onChange={this.setContestParticipantsGroups} addon type="checkbox"  />
                                    </InputGroupText>
                                    </InputGroupAddon>
                ...
                <FormGroup row>
                    ...
                    <Col sm={6}>
                        <Input onChange={this.setContestType} type="select" name="selectContestType" id="contest_type">
                ...
                <FormGroup row>
                    ...
                    <Col sm={6}>
                        <Input  onChange={this.onCourseChange} type="select" name="selectCours" id="contest_title">                        
                            {!this.state.courses
                                ? null
                ...
                <FormGroup row>
                    ...
                    <Col sm={6}>
                        <InputGroup key={lang}>
                            <InputGroupAddon addonType="prepend">
                            <InputGroupText>
                                <Input onChange={this.onToggleAllProgrammingLanguages} addon type="checkbox" checked={Object.values(this.state.programmingLanguagesEnabled).every(x => x)} />
                            </InputGroupText>
                            <InputGroupText>
                                Выбрать все
                            </InputGroupText>
                        ...
                        {!this.state.allowLanguages
                            ...
                            : this.state.allowLanguages.map((lang) => (
                                <InputGroup key={lang}>
                                    <InputGroupAddon addonType="prepend">
                                    <InputGroupText>
                                        <Input name={lang} onChange={this.onChangeAllowLanguagesList} addon type="checkbox" checked={this.state.programmingLanguagesEnabled[lang]} />
                                    </InputGroupText>
                                    <InputGroupText>
                                        {lang}
                                    </InputGroupText>
                ...
                <FormGroup row>
                    ...
                    <Col sm={6}>
                        <Input type="textarea" name="text" id="info"  value={this.state.info} onChange={this.setInfo} />
                    </Col>
                </FormGroup>
                <FormGroup row>
